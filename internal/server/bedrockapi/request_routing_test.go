...
func TestInvokePolicyRouting(t *testing.T) {
	for _, stream := range []bool{false, true} {
		for _, surface := range []string{"system", "tool", "numeric"} {
			t.Run(surface+map[bool]string{false: "/complete", true: "/stream"}[stream], func(t *testing.T) {
				...
				f.Private.Fail = true
				raw := routingtest.Body("bedrock", surface, stream)
				rec := httptest.NewRecorder()
				NewInvokeHandlerMetrics(f.Router, f.Holder, nil, nil, nil, stream).ServeHTTP(rec, routingtest.Request("/model/premium/invoke", raw))
...
func TestBedrockCountPolicyRouting(t *testing.T) {
	for _, action := range []v1alpha1.SensitiveDataAction{v1alpha1.InternalOnly, v1alpha1.Block} {
		t.Run(string(action), func(t *testing.T) {
			...
			inner := routingtest.Body("bedrock", "numeric", false)
			rec := httptest.NewRecorder()
			NewCountTokensHandler(f.Router, f.Holder).ServeHTTP(rec, routingtest.Request("/model/premium/count-tokens", countTokensBody(t, inner)))
			if rec.Code != 200 {
			...
			if action == v1alpha1.InternalOnly && (len(f.Private.Calls) != 1 || f.Private.Calls[0].Body != inner) {
				t.Fatal("count did not inspect decoded body and use approved route")
			}
