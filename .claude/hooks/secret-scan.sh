...
# Files to skip (examples, docs, this scanner, lockfiles, the scanner's own
# pattern-test files which necessarily contain secret-shaped sample strings)
SKIP_PATTERNS=('.env.example' '*secret-scan.sh' '*.md' 'go.sum' 'examples/config.json' 'tests/fixtures/*' 'tests/hooks/*')
...
if [ "$SECRETS_FOUND" -eq 1 ]; then
    ...
    echo "[secret-scan] inferplane never stores secrets inline — use env:/file:/secret: refs."
    exit 1
fi
