...
- 최신 Bedrock 모델(Opus 4.7/4.8, Fable 5, Sonnet 5, Mythos — `providers/bedrock/thinking.go`의 allow-list)은 Claude Code가 아직 보내는 구식 extended-thinking 형태(`thinking: {"type":"enabled","budget_tokens":N}`)를 400으로 거부합니다; `toInvokeBody`가 이 모델들에 한해서만 `thinking: {"type":"adaptive"}` + 최상위 `output_config: {"effort":...}`로 재작성하고, 그 외 모델의 `thinking` 필드는 그대로 통과시킵니다(ADR-022 — CLI/모델 스키마 차이를 메우는 임시 호환 계층, 영구 해법 아님).
...
- `providers/bedrock/thinking.go` — allow-list된 모델에 대한 legacy `thinking` → adaptive+`effort` 재작성(ADR-022)
- `pkg/schema/extra.go` — 미지 필드 보존 + 대소문자 충돌 거부

### 5. 상호 참조
- 관련 모듈: `internal/router`(해석/폴백), `internal/openai`(변환), `internal/keystore`(팀 레코드 가드레일 오버라이드)
- 관련 ADR: docs/decisions/ADR-019-bedrock-guardrails-data-plane.md, docs/decisions/ADR-022-bedrock-legacy-thinking-rewrite.md
- 관련 런북: docs/runbooks/
